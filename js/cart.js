...
function updateProductQuantity(productId, newAmount, newTotal) {
  ...
  const priceElement = document.querySelector(
    `[data-product-id="${productId}"] .cart_product_price`
  );
...
function removeProductFromDOM(productId) {
  const productElement = document.querySelector(
    `.cart_product[data-product-id="${productId}"]`
  );
  if (productElement) {
  ...
  fetchWithRetry("src/addRemoveCart.php", {
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        const existingProductIds = data.cart.items.map((item) =>
          item.id.toString()
        );
    ...
    .then((data) => {
      if (data.success) {
        ...
        updateProductCounter(
          data.cart.items.find((item) => item.id == productId)?.amount || 0
        );
      } else {
        alert(data.message || "Ошибка при работе с корзиной");
      }
