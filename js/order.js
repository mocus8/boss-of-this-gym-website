...
document
    .getElementById("order-cancel-modal-exit")
    ?.addEventListener("click", function () {
...
document
    .getElementById("order-cancel-modal-submit")
    ?.addEventListener("click", async function (e) {
        ...
        if (!orderId || !orderId.match(/^\d+$/)) {
            ...
            headerModal.open(
                "Не удалось определить номер заказа. Пожалуйста, обновите страницу и попробуйте снова."
            );
        ...
        if (!errorModal) {
            headerModal.open(
                "Ошибка данных заказа, обновите страницу и попробуйте еще раз"
            );
            return;
        }
        ...
        if (!orderTitle || !orderActions) {
            ...
            // Потом нормально логировать
            console.error(
                "[Order cancel handler] Cant find orderTitle or orderActions in cancel form:",
                orderId
            );
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (
                    response.status === 409 &&
                    result.error === "ORDER_ALREADY_CANCELLED"
                ) {
                    closeModal("order-cancel-modal");
    ...
    ?.addEventListener("click", async function (e) {
        ...
        try {
            ...
            if (!result.confirmation_url) {
                ...
                errorModal.open(
                    "Ошибка, не получена ссылка для оплаты, попробуйте еще раз."
                );
            ...
            if (error.name === "AbortError") {
                ...
                // Если ошибки связаны с сетью
                errorModal.open(
                    "Нет соединения с сервером. Проверьте интернет соеденение."
                );
            } else {
                // Другие ошибки
                errorModal.open(
